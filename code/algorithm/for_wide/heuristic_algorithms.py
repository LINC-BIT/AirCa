...
#                 item = self.problem.cargo_items.iloc[i]
#                 # 获取可用舱位（未被其他货物占用的、类型匹配的）
#                 available = self.get_available_holds(item, [False] * n_holds)
#                 if available:
#                     mutated[i] = np.random.choice(available + [-1])
#                 else:
#                     mutated[i] = -1
#
#         return mutated
...
#                         item = self.problem.cargo_items.iloc[j]
#                         available = self.get_available_holds(item, [False] * n_holds)
#                         if available:
#                             new_pos[j] = np.random.choice(available + [-1])
#
...
#             nests.append(nest)
#
#         fitness = [self._fitness(n) for n in nests]
#         best_idx = np.argmin(fitness)
#         best_nest = nests[best_idx].copy()
#         best_fitness = fitness[best_idx]
#
...
#
#             # 发现概率：抛弃较差的解
#             worst_indices = np.argsort(fitness)[-int(self.pa * self.n_nests):]
...
#             available = self.get_available_holds(item, [False] * n_holds)
#             if available:
#                 new_nest[i] = np.random.choice(available + [-1])
#             else:
#                 new_nest[i] = -1
#
#         return new_nest
...
#
#     def __init__(self, problem, segment_type='single',
#                  n_ants=20, iterations=50, alpha=1.0, beta=2.0, rho=0.5,
#                  time_limit=30):
#         super().__init__(problem, segment_type)
#         self.n_ants = n_ants
#         self.iterations = iterations
...
#
#         # 初始化信息素矩阵
#         pheromone = np.ones((n_items, n_holds + 1)) * 0.1  # +1 for not loading
#
#         # 计算启发式信息
#         heuristic = self._compute_heuristic()
#
#         best_solution = self._greedy_init()
...
#         )
#
#         heuristic = np.zeros((n_items, n_holds + 1))
#
#         for i in range(n_items):
#             item = self.problem.cargo_items.iloc[i]
#             for j in range(n_holds):
...
#                     hold = self.problem.holds[j]
#
#                     if item['weight'] > hold['max_weight'] or not self.is_hold_compatible(item, hold):
#                         probs.append(0)
#                     else:
#                         prob = (pheromone[i, j] ** self.alpha) * (heuristic[i, j] ** self.beta)
#                         probs.append(prob)
#                         valid_holds.append(j)
#
#             # 不装载的概率
#             prob_not_load = (pheromone[i, n_holds] ** self.alpha) * (heuristic[i, n_holds] ** self.beta)
#             probs.append(prob_not_load)
#
...
#
#     def __init__(self, problem, segment_type='single',
#                  colony_size=30, iterations=50, limit=20, time_limit=30):
#         super().__init__(problem, segment_type)
#         self.colony_size = colony_size
#         self.iterations = iterations
#         self.limit = limit
...
#
#         # 初始化食物源
#         n_foods = self.colony_size // 2
#         foods = []
#         for i in range(n_foods):
#             if i == 0:
#                 food = np.array(self._greedy_init())
#             else:
#                 food = np.array(self.repair_solution(self.generate_random_solution()))
#             foods.append(food)
#
#         fitness = np.array([self._fitness(f) for f in foods])
...
#                 i = np.random.choice(n_foods, p=probs)
#                 new_food = self._employed_bee(foods[i], foods, n_holds)
#                 new_fitness = self._fitness(new_food)
#
#                 if new_fitness < fitness[i]:
...
#             for i in range(n_foods):
#                 if trials[i] > self.limit:
#                     foods[i] = np.array(self.repair_solution(self.generate_random_solution()))
...
#                 available = self.get_available_holds(item, [False] * n_holds)
#                 if available:
#                     new_food[i] = np.random.choice(available + [-1])
#                 else:
#                     new_food[i] = -1
#
#         return np.array(self.repair_solution(list(new_food)))
...
#
#         # 两个子种群
#         n_land1 = int(self.pop_size * 0.5)
#         n_land2 = self.pop_size - n_land1
#
#         land1 = np.zeros((n_land1, n_items), dtype=np.int32)
#         land2 = np.zeros((n_land2, n_items), dtype=np.int32)
#
...
#                     new_butterfly = land1[i].copy()
#                     idx = np.random.randint(n_items)
#                     item = self.problem.cargo_items.iloc[idx]
#                     available = self.get_available_holds(item, [False] * n_holds)
#                     if available:
#                         new_butterfly[idx] = np.random.choice(available + [-1])
#
#                 new_butterfly = np.array(self.repair_solution(list(new_butterfly)))
#                 new_fitness = self._fitness(list(new_butterfly))
#
    ...
    def _construct_solution(self, pheromone, heuristic):
        ...
        for i in range(n_items):
            ...
            if probs.sum() > 0:
                ...
                # 如果所有概率为0，随机选择可用舱位
                available = [j for j in range(n_holds) if not hold_occupied[j]]
                if available:
