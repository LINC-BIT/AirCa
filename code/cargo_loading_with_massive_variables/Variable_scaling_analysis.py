...
def main():
    parser = argparse.ArgumentParser(description="A320 Experiment 1: Variable Scaling Analysis (single only)")
    ...
    parser.add_argument("--time-limit", type=int, default=120, help="Solver time limit passed to algorithms (seconds).")
    parser.add_argument("--algo-timeout", type=int, default=120, help="Hard timeout per algorithm run (seconds).")
    parser.add_argument("--split-thresholds", type=str, default="100,50,25,10",
                        help="Comma-separated split thresholds in kg.")
    ...
    print("=" * 90)
    print("A320 - Experiment 1: Variable Scaling Analysis")
    print("=" * 90)
    ...
    print(f"algo_timeout       : {args.algo_timeout}s")
    print(f"split_thresholds   : {args.split_thresholds}")
    ...
    split_thresholds = parse_int_list(args.split_thresholds)
    ...
    for threshold in split_thresholds:
        ...
        for fi, flight_num in enumerate(flights, 1):
            ...
            split_df = loader.split_cargo(
                original,
                max_piece_weight=threshold,
                min_piece_weight=max(5, threshold // 5),
            )
            n_items = len(split_df)
            print(f"  Flight [{fi}/{len(flights)}] {flight_num}: {len(original)} -> {n_items} items")
            if n_items == 0:
                continue
            ...
            for algo_class in algo_classes:
                ...
                payload, err = run_with_timeout(run_single_algorithm, args.algo_timeout, algo_class, problem, args.time_limit)
                if payload is None:
                    ...
                    results.append({
                        ...
                        "n_items": n_items,
                        "cg_gap_percent": None,
                        "solve_time": args.algo_timeout if status == "TIMEOUT" else None,
                        "memory_mb": None,
                        "feasible": None,
                        "n_violations": None,
        ...
        pd.DataFrame(results).to_csv(os.path.join(args.output_dir, "a320_variable_scaling_intermediate.csv"), index=False)

    df = pd.DataFrame(results)
    out_csv = os.path.join(args.output_dir, "a320_variable_scaling_results.csv")
